...
export function setUserInfo(user) {
    ...
    if (user && user.user && user.user.course_purchased)
        delete user.user.course_purchased;

    document.cookie = `${"user"}=${JSON.stringify(user.user)};path=/`;
...
export function setLoadingFlag(user) {
    return {
        type: SET_LOADING,
        data: user.flag
    }
            ...
            if (res.data.data['token']) {
                if (cart !== undefined && cart !== "undefined") {
                    ...
                    if (res.data.data['user'].cart.length > 0) {
                        res.data.data['user'].cart.map(value => {
                            let color = value.color;
                            if (value.item && value.item.variants) {
                                value.item.variants.map((data) => {
                                    if (data.color == color) {
                                        cartArray.push({
                                            name: value.item.title,
